...
for provider in taku pangle; do
  callback_location="location ^~ /app-api/skit/ad-callback/${provider}/ {"
  callback_line="$(grep -nF "${callback_location}" "${nginx_config}" | cut -d: -f1 || true)"
  [ -n "${callback_line}" ] || fail "dedicated ${provider} callback location is missing"
  [ "${callback_line}" -lt "${generic_line}" ] \
    || fail "${provider} callback location must precede the generic regex location"

  callback_block="$(tail -n "+${callback_line}" "${nginx_config}" | sed -n '1,/^  }$/p')"
  grep -Eq '^    access_log off;$' <<<"${callback_block}" \
    || fail "${provider} callback access logging must be disabled"
  grep -Eq '^    error_log /dev/null crit;$' <<<"${callback_block}" \
    || fail "${provider} callback upstream errors must not echo the secret route or query"
  grep -Eq '^    proxy_pass http://backend:48080;$' <<<"${callback_block}" \
    || fail "${provider} proxy_pass must omit a replacement URI so the untouched path and query reach backend"
  grep -Eq '^    proxy_set_header X-Real-IP \$skit_client_ip;$' <<<"${callback_block}" \
    || fail "${provider} callback proxy must forward the canonical client IP"
  grep -Eq '^    proxy_set_header X-Forwarded-For \$skit_forwarded_for;$' <<<"${callback_block}" \
    || fail "${provider} callback proxy must forward the canonical forwarding chain"
done
